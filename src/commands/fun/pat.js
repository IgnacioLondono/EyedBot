...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            const mediaPromise = fetchInteractionGif('pat');
            const countsPromise = incrementMentionCount('pat', interaction.guild?.id || null, user.id).catch(() => ({ total: null, actionCount: null }));
            const [media, counts] = await Promise.all([mediaPromise, countsPromise]);
            ...
            if (Number.isFinite(counts?.total) && Number.isFinite(counts?.actionCount)) {
                embed.addFields({
                    name: '📊 Conteo',
                    value: `Menciones a ${user}: **${counts.total}** total (**${counts.actionCount}** en pat)`,
                    inline: false
                });
            }
            ...
            const components = createReturnComponents('pat', interaction.user.id, user.id);
